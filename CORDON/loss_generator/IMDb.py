...
def drop_pk(query_indicators, has_pred_on_title_list, predicates, sample_encs, table2vec, column2vec,
	...
	for qid, query in enumerate(predicates):
		...
		if len(ori_table) == 1 or (has_pred_on_title is False) or query_indicators[qid] == 0:
		...
		# the case for >= 2 tables, we first separate two subqueries
	...
	return predicate_list, sample_list, predicate_mask_list, sample_mask_list, indicators
...
def expand_query_add_filter(break_query_indicators, predicates, sample_encs, column_min_max_vals, table2vec, column2vec,
                            ...
                            ori_predicates, ori_tables, ori_samples, table_to_df, rs):
	len_colvec = len(column2vec)
...
def expand_k_query_add_filter(break_query_indicators, k, predicates, sample_encs, column_min_max_vals, table2vec, column2vec,
	...
	### helper function for psedual labeling
	...
	for qid, query in enumerate(predicates):
		...
		if is_all_one:
			...
			for _ in range(k):
				...
				## compute samples for new queries
				bitmap_list = get_bitmap.compute_bitmap(table_to_df, [chosen_table, chosen_table], [pred1_a_q, pred2_a_q])
				sample_enc_list = encode_samples3([ori_table, ori_table], [ori_sample, ori_sample], table2vec,
				                                          [chosen_table, chosen_table], bitmap_list)
